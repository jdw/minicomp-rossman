...
from utility.test_train_split import get_train_test_split

from models.default_pipeline import DefaultPipeline
...
for p in param_grid:
    model = DefaultPipeline(xg.XGBRegressor(objective='reg:squarederror',
                                            booster='gbtree',
                                            gamma=p["gamma"],
    ...
    print(p, metric( model.predict(trainX), trainY.values ), metric( model.predict(testX), testY.values ))
    param_result.append((p, metric( model.predict(trainX), trainY.values ), metric( model.predict(testX), testY.values )))
